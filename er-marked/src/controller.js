...
define(
    function (require) {
         ...
         * @protected
         */
        // MARK 返回一个promise对象，在require好对应的anction构造函数后，会用对应action的一个示例来resolve这个promise对象。
        exports.loadAction = function (actionContext) {
            ...
            var callback = function (SpecificAction) {
                ...
                if (typeof SpecificAction === 'function') {
                    // MARK promise的规范里，promise只支持用一个参数来进行resole，这个支持两个
                ...
                // 处理Action工厂，Action工厂是一个对象，它能生产出一个Action对象，有`createRuntimeAction`方法的对象即为Action工厂
                    ...
                    // MARK createRuntimeAction(actionContext) 的返回值应该是个promise对象(是用一个action来resolve的)
         ...
         * @protected
         */
        // MARK 调用action.enter(actionContext)返回一个promise对象(这个对象表示进入完成和失败)
        exports.enterAction = function (action, actionContext) {
            ...
            // MARK 这里转到 Action 类里去执行了
            console.log(333333333, actionContext)
        ...
        exports.forward = function (url, container, options, isChildAction) {
            ...
            // 把url的hash中的查询数据也填充到 actionContext.args 中
         ...
         * @return {meta.Promise}
         */
        // MARK 加载对应的Action，并action.enter()
