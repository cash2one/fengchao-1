...
define(
    function (require) {
         ...
         */
        // MARK 创建model示例，并调用model的load进行加载(promise)，完成后，调用forwardToView
        exports.enter = function (actionContext) {
            ...
            // MARK mode.load数据，然后model.prepare对数据处理(比如格式化)，然后调用 action.forwardToView来渲染
            ...
            // MARK 这里用 当前的 Action 实例来resolve
            // MARK 如果action的model没有load方法，说明不用加载数据，数据就在model创建的时候以参数的形式穿进去的。
            this.forwardToView();
         ...
         * @fires entercomplete
         */
        // MARK 创建 view，并render ,然后 this.initBehavior()
         ...
         * @fires leave
         */
        // MARK 销毁 和 这个Action关联的 model和view
