...
define(
    function (require) {
         ...
         * @ignore
         */
        // MARK 返回一个promise对象，调用 option.retrieve(model, option) 获取数据，并且把数据添加到model中，返回的promise对象用 addDataToModel() 的返回值来resolve
         ...
         * @ignore
         */
        // MARK 注意这里用 then 链式调用实现的 顺序加载数据
         ...
         * @ignore
         */
        // MARK 注意这里用 Deferred.all 实现的 并行加载数据
         ...
         * @ignore
         */
        // MARK 主要是注意 datasource.retrieve 方法
         ...
         * @param {meta.Promise} worker 工作对象
         */
        // MARK 加入队列中，并且在worker完成后自动从队列里移除
        ...
        function forwardToPrepare() {
            ...
            try {
                // MARK model.prepare 需要在model示例上自定义
         ...
         */
        // MARK 根据datasoure来获取数据，并把数据加入到model里，添加是否成功保存在result里，然后调用forwardToPrepare(result)
        exports.load = function () {
            try {
                var datasource = this.getDatasource();
                var loading = load(this, datasource);
                // MARK forwardToPrepare 根据用户自定义的prepare函数，返回值可能是也可能不是 个 promise
         ...
         * @fires change
         */
        // MARK 往model里填充数据，并根据是否 silent 来触发 它的change事件(事件对象为 变化 的列表)
