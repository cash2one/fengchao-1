...
// MARK 这个模块主要是 扫描页面上的所有元素，根据元素名和属性来初始 判断组件的类型和参数，然后一次创建组件
define(
    function (require) {
        var lib = require('./lib');
        console.log('lib', lib);
        ...
        main.init = function (wrap, options) {
            ...
            var uiPrefix = main.getConfig('uiPrefix'); // MARK 'data-ui'
            var extPrefix = main.getConfig('extensionPrefix'); // MARK 'data-ui-extension'
            var customElementPrefix = main.getConfig('customElementPrefix'); // MARK 'esui'
            ...
            for (var i = 0, len = elements.length; i < len; i++) {
                ...
                if (type) {
                    ...
                    // 创建控件
                    var control = main.create(type, controlOptions); // MARK 先创建 extensions，然后创建控件本身
                    if (control) {
         ...
         */
        // MARK 扫描esui全部的注册的规则，然后和 对应控件比较，看它是否需要那种规则来验证。如果是，
        // 就注入到 那个控件的验证器 里边。
        main.createRulesByControl = function (control) {
            var rules = [];
            for (var i = 0; i < ruleClasses.length; i++) {
                var RuleClass = ruleClasses[i].type;
                // MARK 每个控件的需要验证的属性也是绑定在自己同名的属性上的。比如 maxLength,
                // 通过这个属性 来把对应的 验证器到 它 的 validity 里边
